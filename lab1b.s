...
# ----------------------------------------------------------
	...
	.text
	.set noreorder
main:	li	$s0, 0x4fa917e0	# initialize "seed"
...
break:  lw 	$t0, 0($s1)		# Performs the operations given in the instructions
	sll 	$t3, $t0, 3		# Multiply by 8
	sra 	$t4, $t0, 6		
	subu 	$t5, $t3, $t4	
	subu 	$t0, $t5, $t2	

	xor 	$v0, $t0, 0x141703d1
	sw  	$t0, 0($s1)		# Stores the new seed
...
then:	sb 	$zero, 0($a1)	 	# Byte pointed to by bytearr = 0
	lw 	$t0, -4($fp)		# Load x
	sw 	$t0, -16($fp)		# r = x
	addiu 	$v0, $t0, 0		# Return x
	b 	exit

else:	addiu 	$a0, $a0, 4		# Next word in wordarr
	addiu 	$a1, $a1, 1		# Next byte in bytearr
	bal 	decode			# decode( wordarr+, bytearr+ )
	addiu 	$t0, $v0, 0		
	sw 	$t0, -20($fp)		# Store y to stack
	
	lw 	$t0, -4($fp)		# Load all necessary variables from the stack
	lw 	$t1, -8($fp)	 	
	lw 	$t2, -20($fp)	
	lw 	$t3, 0($t1)		# Load word from wordarr pointer

	subu 	$t4, $t0, $t2		# $t4 = (x - y)
	xor 	$t5, $t4, $t3		# $t5 = (x - y) xor wordarr
	sw 	$t5, -24($fp)		# Store m to stack

	srl 	$t5, $t5, 7		# Shift right to start at seventh bit
	lw 	$t0, -12($fp)		# Load bytearr pointer
	sb 	$t5, 0($t0)		# Store [14:7] byte to bytearr
